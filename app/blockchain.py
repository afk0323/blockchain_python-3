...
class Blockchain:
    ...
    def chain_is_valid(self, chain):
            ...
            for i in range(1, len(chain)):
                ...
                if current_block['previous_block_hash'] != prev_block['hash']: #채우시오 : i가 1이 아니거나 현재 블럭의 previous_hash값과 이전 블럭의 hash값이 같은지 비교:
                    validChain = False
    ...
    def create_merkle_tree(self, transactions):
        if len(transactions) == 0:
            ...
            transactions.append(transactions[0]) #채우시오 : 가장 마지막 tx를 trasactions에 추가
            new_level = []
            
            for i in range(0, len(transactions), 2): # 채우시오 : 0부터 len(tx)까지
            ...
            while len(transactions) > 1:
                ...
                for i in range(0, len(transactions), 2): #채우시오, 채우시오, 채우시오): 0부터 len(tx)까지
                    left = left = transactions[i] #채우시오 : i를 활용하여 왼쪽 노드
